...
function CountSession({ count: initialCount, onFinish, categories, catAr }) {
 ...
 {count.status === 'applied' ? ' تقرير جرد مؤرشف' : ' جلسة جرد نشطة'}
 </p>
 ...
 {diffCount > 0 ? ` ${diffCount} فرق` : ' لا فروقات'}
 </p>
 </div>
 </div>
 ...
 {[{ id: 'all', l: 'الكل' }, { id: 'uncounted', l: '⬜ لم تُعدّ' }, { id: 'counted', l: ' تم العد' }, { id: 'diff', l: ' فيها فرق' }].map(f => (
...
function AdminInventoryCount() {
 ...
 {diffItems > 0 && <span className="text-amber-600"> {diffItems} فرق</span>}
 </div>
 </div>
 <div className="flex gap-2">
