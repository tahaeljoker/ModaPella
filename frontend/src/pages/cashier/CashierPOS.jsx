...
function CashierPOS() {
 ...
 showToast(` تم مسح الكود [${sku}] — اضغط الخيار بالأسفل لربطه بمنتج`, 'error');
 }
 })
 .catch(() => {
 setUnlinkedBarcode(sku);
 ...
 showToast(' يجب إدخال اسم العميل ورقم هاتفه لتسجيل البيع الآجل (الدين)', 'error');
 return;
 }
 ...
 showToast(' تم حفظ الفاتورة محلياً لعدم اتصالك بالإنترنت', 'error');
 ...
 saveOfflineSales([...offlineSales, { id: offlineId, payload: salePayload }]);
 setCompletedOrder(fakeOrder);
 showToast(' تم حفظ الفاتورة محلياً لعدم القدرة على الوصول للسيرفر', 'error');
 ...
 <span>{!isOnline ? ' أنت تعمل بدون إنترنت حالياً (الوضع أوفلاين نشط)' : ' تم استعادة الاتصال بالإنترنت'}</span>
 ...
 <span className="text-[10px] text-burgundy/30 opacity-0 group-hover/price:opacity-100 transition"></span>
 </button>
 )}
 {!item.allowDiscount && (
 ...
 <span> تذكير: بعض المنتجات في الفاتورة غير خاضعة للخصم عادةً</span>
 </p>
 </div>
 )}
 ...
 <span className="text-amber-700 font-extrabold"> الديون المتبقية: {EGP(matchedCustomer.debt)}</span>
 )}
 </div>
 </div>
 )}
 ...
 يرجى إدخال اسم العميل ورقم هاتفه أعلاه لتسجيل الدين باسمه.
 </p>
 )}
 </div>
 )}
